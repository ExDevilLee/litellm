...
export function GuardrailSettingsView({
  ...
  const content = isEmpty ? (
    <span className="block text-gray-500">{t("No guardrails configured")}</span>
  ) : (
    <div className="flex flex-col gap-4">
      <div>
        ...
        {killSwitchOn ? (
          <Tag color="gold">{t("Bypassed for this team")}</Tag>
          ...
          <span className="block text-sm text-gray-500">{t("None configured")}</span>
        )}
      </div>
      <div>
        <span className="block text-sm font-medium text-gray-700 mb-2">{t("Team-specific")}</span>
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <span className="block font-semibold text-gray-900">{t("Guardrails Settings")}</span>
            <span className="block text-xs text-gray-500">
              {t("Global and team-specific guardrails applied to this team")}
            </span>
          </div>
        </div>
        {content}
  ...
  return (
    <div className={`${className}`}>
      <span className="block font-medium text-gray-900 mb-3">{t("Guardrails Settings")}</span>
      {content}
    </div>
  );
}
