...
const BulkCreateUsersButton: React.FC<BulkCreateUsersProps> = ({
  ...
  const handleFileUpload = (file: File) => {
    ...
    if (file.type !== "text/csv" && !file.name.endsWith(".csv")) {
      setFileError(t("Invalid file type: {0}. Please upload a CSV file (.csv extension).", file.name));
    ...
    if (file.size > 5 * 1024 * 1024) {
      setFileError(
        t("File is too large ({0} MB). Please upload a CSV file smaller than 5MB.", (file.size / (1024 * 1024)).toFixed(1)),
      );
      return;
    }
    ...
    Papa.parse(file, {
      complete: (results) => {
        ...
        if (results.data.length === 1) {
          setCsvStructureError(t("The CSV file only contains headers but no user data. Please add user data to your CSV."));
          setParsedData([]);
          return;
        }
        ...
        if (headers.length === 0 || (headers.length === 1 && headers[0] === "")) {
          setCsvStructureError(t("The CSV file doesn't contain any column headers. Please make sure your CSV has headers."));
          setParsedData([]);
          return;
        }
        ...
        if (missingColumns.length > 0) {
          setCsvStructureError(
            t("Your CSV is missing these required columns: {0}. Please add these columns to your CSV file.", missingColumns.join(", ")),
          );
          setParsedData([]);
          return;
        }
        ...
        try {
            ...
            .map((row: any, index: number) => {
              ...
              if (row.length < headers.length) {
                return {
                  rowNumber: index + 2,
                  isValid: false,
                  error: t("Row {0} has fewer columns than the header row. Please ensure all data is properly formatted.", index + 2),
                  user_email: "",
                  user_role: "",
                } as UserData;
              }
              ...
              if (!user.user_role) {
                ...
                if (!validRoles.includes(user.user_role)) {
                  errors.push(t('Invalid role "{0}". Must be one of: {1}', user.user_role, validRoles.join(", ")));
                }
              }
              ...
              if (user.max_budget && user.max_budget.toString().trim() !== "") {
                if (isNaN(parseFloat(user.max_budget.toString()))) {
                  errors.push(t('Max budget "{0}" must be a number', user.max_budget));
              ...
              if (user.budget_duration && !user.budget_duration.match(/^\d+[dhmwy]$|^\d+mo$/)) {
                errors.push(
                  t('Invalid budget duration format "{0}". Use format like "30d", "1mo", "2w", "6h"', user.budget_duration),
                );
              }
              ...
              if (user.teams && typeof user.teams === "string") {
                ...
                if (teams && teams.length > 0) {
                  ...
                  if (invalidTeams.length > 0) {
                    errors.push(t("Unknown team(s): {0}", invalidTeams.join(", ")));
                  }
                }
              }
          ...
          if (userData.length === 0) {
            ...
            setParseError(
              t("Found {0} row(s) with errors out of {1} total rows. Please correct them before proceeding.", userData.length - validData.length, userData.length),
            );
          } else {
            NotificationsManager.success(t("Successfully parsed {0} users", validData.length));
          }
        } catch (error: unknown) {
          const errorMessage = error instanceof Error ? error.message : t("Unknown error");
          setParseError(t("Error parsing CSV: {0}", errorMessage));
          setParsedData([]);
        }
      },
      error: (error) => {
        setParseError(t("Failed to parse CSV file: {0}", error.message));
        setParsedData([]);
      },
      header: false,
    });
    ...
    <>
      ...
      <Dialog open={isModalVisible} onOpenChange={(open) => !open && setIsModalVisible(false)}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
          <DialogHeader>
            <DialogTitle>{t("Bulk Invite Users")}</DialogTitle>
          </DialogHeader>
            ...
            {parsedData.length === 0 ? (
              <div className="mb-6">
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="text-lg font-medium">{t("Download and fill the template")}</h3>
                </div>

                <div className="ml-11 mb-6">
                  <p className="mb-4">{t("Add multiple users at once by following these steps:")}</p>
                  <ol className="list-decimal list-inside space-y-2 ml-2 mb-4">
                    <li>{t("Download our CSV template")}</li>
                    <li>{t("Add your users' information to the spreadsheet")}</li>
                    <li>{t("Save the file and upload it here")}</li>
                    <li>{t("After creation, download the results file containing the Virtual Keys for each user")}</li>
                  </ol>

                  <div className="bg-gray-50 p-4 rounded-md border border-gray-200 mb-4">
                    <h4 className="font-medium mb-2">{t("Template Column Names")}</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="flex items-start">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{t("User's email address (required)")}</p>
                        </div>
                      </div>
                      <div className="flex items-start">
                        <div className="w-3 h-3 rounded-full bg-red-500 mt-1.5 mr-2 shrink-0"></div>
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{t('Comma-separated team IDs (e.g., "team-1,team-2")')}</p>
                        </div>
                      </div>
                      <div className="flex items-start">
                        <div className="w-3 h-3 rounded-full bg-gray-300 mt-1.5 mr-2 shrink-0"></div>
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{t('Maximum budget as a number (e.g., "100")')}</p>
                        </div>
                      </div>
                      <div className="flex items-start">
                        <div className="w-3 h-3 rounded-full bg-gray-300 mt-1.5 mr-2 shrink-0"></div>
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{t('Budget reset period (e.g., "30d", "1mo")')}</p>
                        </div>
                      </div>
                      <div className="flex items-start">
                        <div className="w-3 h-3 rounded-full bg-gray-300 mt-1.5 mr-2 shrink-0"></div>
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {t('Comma-separated allowed models (e.g., "gpt-3.5-turbo,gpt-4")')}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="text-lg font-medium">{t("Upload your completed CSV")}</h3>
                </div>
                  ...
                  {selectedFile ? (
                    ...
                    >
                      ...
                      >
                        ...
                        <p className="mb-1">{t("Drag and drop your CSV file here")}</p>
                        <p className="text-sm text-gray-500 mb-3">{t("or")}</p>
                        <span className={buttonVariants({ variant: "outline", size: "sm" })}>{t("Browse files")}</span>
                        <p className="text-xs text-gray-500 mt-4">{t("Only CSV files (.csv) are supported")}</p>
                      </div>
                    </label>
                  )}
                  ...
                  {csvStructureError && (
                    <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-md">
                      <div className="flex items-start">
                        ...
                        <div className="min-w-0">
                          <strong className="text-yellow-800">{t("CSV Structure Error")}</strong>
                          <p className="text-yellow-700 mt-1 mb-0 break-words">{csvStructureError}</p>
                          <p className="text-yellow-700 mt-2 mb-0">
                            {t("Please download our template and ensure your CSV follows the required format.")}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="text-lg font-medium">
                    {parsedData.some((user) => user.status === "success" || user.status === "failed")
                      ? t("User Creation Results")
                      : t("Review and create users")}
                  </h3>
                </div>
                      ...
                      <div className="min-w-0">
                        ...
                        {parsedData.some((user) => !user.isValid) && (
                          <ul className="mt-2 list-disc list-inside text-red-600 text-sm">
                            <li>{t("Check the table below for specific errors in each row")}</li>
                            <li>
                              {t("Common issues include invalid email formats, missing required fields, or incorrect role values")}
                            </li>
                            <li>{t("Fix these issues in your CSV file and upload again")}</li>
                          </ul>
                        )}
                      </div>
                    </div>
                ...
                <div className="ml-11">
                  <div className="flex justify-between items-center mb-3">
                    <div className="flex items-center">
                      {parsedData.some((user) => user.status === "success" || user.status === "failed") ? (
                        <div className="flex items-center">
                          <p className="text-lg font-medium mr-3">{t("Creation Summary")}</p>
                          <p className="text-sm bg-green-100 text-green-800 px-2 py-1 rounded-sm mr-2">
                            {t("{0} Successful", parsedData.filter((d) => d.status === "success").length)}
                          </p>
                          {parsedData.some((d) => d.status === "failed") && (
                            <p className="text-sm bg-red-100 text-red-800 px-2 py-1 rounded-sm">
                              {t("{0} Failed", parsedData.filter((d) => d.status === "failed").length)}
                            </p>
                          )}
                        </div>
                      ) : (
                        <div className="flex items-center">
                          <p className="text-lg font-medium mr-3">{t("User Preview")}</p>
                          <p className="text-sm bg-blue-100 text-blue-800 px-2 py-1 rounded-sm">
                            {t("{0} of {1} users valid", parsedData.filter((d) => d.isValid).length, parsedData.length)}
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    {!parsedData.some((user) => user.status === "success" || user.status === "failed") && (
                      <div className="flex space-x-3">
                        ...
                        >
                          {isProcessing
                            ? t("Creating...")
                            : t("Create {0} Users", parsedData.filter((d) => d.isValid).length)}
                        </Button>
                      </div>
                    )}
                  </div>
                  ...
                  {parsedData.some((user) => user.status === "success") && (
                    <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
                      <div className="flex items-start">
                        ...
                        <div>
                          <p className="font-medium text-blue-800">{t("User creation complete")}</p>
                          <p className="block text-sm text-blue-700 mt-1">
                            <span className="font-medium">{t("Next step:")}</span>{" "}
                            {t("Download the credentials file containing Virtual Keys and invitation links. Users will need these Virtual Keys to make LLM requests through LiteLLM.")}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  {pageCount > 1 && (
                    <div className="flex items-center justify-end gap-3 mt-2">
                      <span className="text-sm text-gray-500">
                        {t("Page {0} of {1}", currentPage + 1, pageCount)}
                      </span>
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  {parsedData.some((user) => user.status === "success" || user.status === "failed") && (
                    <div className="flex justify-end mt-4">
                      <Button variant="outline" onClick={resetParsedData} className="mr-3">
                        {t("Start New Bulk Import")}
                      </Button>
                      <Button onClick={downloadResults}>
                        <Download className="size-4" />
                        {t("Download User Credentials")}
                      </Button>
                    </div>
                  )}
                </div>
