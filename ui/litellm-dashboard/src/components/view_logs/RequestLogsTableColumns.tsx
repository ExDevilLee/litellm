...
export const getRequestLogsTableColumns = ({
...
}: RequestLogsTableColumnsDeps): ColumnDef<LogEntry>[] => [
  {
    ...
    header: ({ column }) => <DataTableSortHeader column={column} title={t("Time")} variant="dropdown-tristate" />,
    size: 200,
    enableSorting: true,
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const tooltipParts = [
        sessionLlmCount > 0 && t("{0} LLM", sessionLlmCount),
        sessionAgentCount > 0 && t("{0} Agent", sessionAgentCount),
        sessionMcpCount > 0 && t("{0} MCP", sessionMcpCount),
      ].filter(Boolean);
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <StatusBadge tone={isSuccess ? "success" : "error"} label={isSuccess ? t("Success") : t("Failure")} />;
    },
  },
  {
    id: "session_id",
  ...
  },
  {
    id: "spend",
    accessorKey: "spend",
    header: ({ column }) => <DataTableSortHeader column={column} title={t("Cost")} variant="dropdown-tristate" />,
    size: 110,
    enableSorting: true,
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col items-end">
          ...
          {isMultiCallSession && <span className="text-[10px] text-gray-400">{t("session total")}</span>}
          {mcpCount > 0 && mcpSpend > 0 && (
            <span className="text-[10px] text-amber-600">
              {t("incl. {0} from {1} MCP", getSpendString(mcpSpend), mcpCount)}
            </span>
          )}
        </div>
      );
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableSortHeader column={column} title={t("Duration (s)")} variant="dropdown-tristate" />,
    enableSorting: true,
    meta: { numeric: true },
    cell: ({ row }) => {
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableSortHeader column={column} title={t("TTFT (s)")} variant="dropdown-tristate" />,
    enableSorting: true,
    meta: { numeric: true },
    cell: ({ row }) => {
      const log = row.original;
  ...
  },
  {
    id: "model",
    accessorKey: "model",
    header: ({ column }) => <DataTableSortHeader column={column} title={t("Model")} variant="dropdown-tristate" />,
    size: 200,
    enableSorting: true,
    cell: ({ row }) => {
      const log = row.original;
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableSortHeader column={column} title={t("Tokens")} variant="dropdown-tristate" />,
    size: 140,
    enableSorting: true,
    meta: { numeric: true },
    cell: ({ row }) => {
