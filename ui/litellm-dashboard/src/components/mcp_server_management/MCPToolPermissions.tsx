  ...
  const fetchToolsForServer = async (serverId: string, token: string) => {
    ...
    try {
      ...
      if (response.error) {
        setToolErrors((prev) => ({
          ...prev,
          [serverId]: response.message || t("Failed to fetch tools"),
        }));
      ...
      setToolErrors((prev) => ({ ...prev, [serverId]: t("Failed to fetch tools") }));
      setServerTools((prev) => ({ ...prev, [serverId]: [] }));
    } finally {
            ...
            <div className="p-4">
              ...
              {isLoading && (
                <div className="flex items-center justify-center py-8">
                  <UiLoadingSpinner />
                  <p className="ml-3 text-sm text-gray-500">{t("Loading tools...")}</p>
                </div>
              )}
              ...
              {error && !isLoading && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-lg text-center">
                  <p className="text-sm text-red-600 font-medium">{t("Unable to load tools")}</p>
                      ...
                      <div key={tool.name} className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-sm text-gray-500">- {tool.description || t("No description")}</p>
                          </div>
                        </div>
                      </div>
                    );
              ...
              {!isLoading && !error && tools.length === 0 && (
                <div className="text-center py-6">
                  <p className="text-sm text-gray-500">{t("No tools available")}</p>
                </div>
              )}
            </div>
          </div>
