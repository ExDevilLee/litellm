...
const MCPToolPermissions: React.FC<MCPToolPermissionsProps> = ({
  ...
  const fetchToolsForServer = async (serverId: string, token: string) => {
    ...
    try {
      ...
      if (response.error) {
        setToolErrors((prev) => ({ ...prev, [serverId]: response.message || t("Failed to fetch tools") }));
            ...
            <div className="p-4">
              ...
              {isLoading && (
                <div className="flex items-center justify-center py-8">
                  <Spin size="large" />
                  <Text className="ml-3 text-gray-500">{t("Loading tools...")}</Text>
                </div>
              )}
                      ...
                      <div key={tool.name} className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <Text className="text-sm text-gray-500">- {tool.description || t("No description")}</Text>
                          </div>
                        </div>
                      </div>
                    );
