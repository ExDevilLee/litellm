      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <Tooltip title={t("Recognize related phrasing beyond exact keyword matches by comparing embeddings instead of plain text. Overrides direct keyword matching")}>
              <InfoCircleOutlined className="text-gray-400" />
            </Tooltip>
          </div>
          <Text className="text-gray-500 text-sm">
            {t("Uses same keyword-tier pairs as above and overrides direct keyword matching. Adds latency based on embedding model network request.")}
          </Text>
        </div>
          ...
          <div>
            ...
            {embeddingModelMissing && (
              <Text type="danger" style={{ fontSize: 12 }}>
                {t("An embedding model is required")}
              </Text>
            )}
          </div>
          <div>
