  ...
  if (targets.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">
        {t("No complexity tiers are configured yet, so there is nothing to test.")}
      </p>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <p className="mb-2 text-sm text-muted-foreground">
        {t(
          "Each configured tier routes to a saved model group. Test Connection sends a minimal request through the proxy to each one, exactly as the auto router would.",
        )}
      </p>
          ...
          >
            ...
            <div className="min-w-0 flex-1 text-sm">
              ...
              <span className="text-muted-foreground">
                {"->"} {target.modelGroup}
                {target.mode === "embedding" ? t(" (embedding)") : ""}
              </span>
              {result.status === "error" && (
