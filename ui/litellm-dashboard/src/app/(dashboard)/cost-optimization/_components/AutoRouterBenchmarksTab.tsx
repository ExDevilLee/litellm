    ...
    <Card className="overflow-hidden py-0">
      <div className="grid md:grid-cols-[4fr_3fr_5fr]">
        <div className="flex flex-col justify-center gap-3 p-6">
          <p className="text-sm text-muted-foreground">{t("Total estimated savings")}</p>
        ...
        <div className="flex flex-col justify-center px-6 pb-6 md:py-6">
          <dl className="divide-y text-sm">
            <div className="flex items-baseline justify-between gap-6 py-3">
              <dt className="text-muted-foreground">{t("Actual auto-router spend")}</dt>
              <dd className="font-medium tabular-nums text-foreground">{usd(stats.spend)}</dd>
            </div>
            <div className="flex items-baseline justify-between gap-6 py-3">
              <dt className="text-muted-foreground">{t("Estimated spend at highest-cost model")}</dt>
        ...
        <div className="flex flex-col border-t md:border-t-0 md:border-l">
          <div className="grid flex-1 grid-cols-2 divide-x">
            <div className="flex flex-col justify-center gap-1 px-6 py-4">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("Total sessions")}</p>
              <p className="text-3xl font-semibold text-foreground">{stats.sessions.toLocaleString()}</p>
            </div>
            <div className="flex flex-col justify-center gap-1 px-6 py-4">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("Total turns")}</p>
  ...
  return (
    <div className="flex flex-col gap-1">
      <div
        ...
        role="img"
        aria-label={t("Share of turns by bucket")}
      >
        {segments.map((b) => (
          <div
            key={b.key}
            ...
            title={t("{0}: {1} turns", t(b.label), b.turns.toLocaleString())}
          />
        ))}
      </div>
  ...
  <Table className="border-b">
    <TableHeader>
      <TableRow className="hover:bg-transparent">
        <TableHead className="text-[11px] uppercase tracking-wide">{t("Bucket")}</TableHead>
        <TableHead className="text-right text-[11px] uppercase tracking-wide">{t("Turns")}</TableHead>
        <TableHead className="w-1/2" />
        <TableHead className="text-right text-[11px] uppercase tracking-wide">{t("Hit rate")}</TableHead>
      </TableRow>
    </TableHeader>
    <TableBody>
      {buckets.map((b) => (
      ...
      <div className="grid lg:grid-cols-[1fr_3fr]">
        <div className="flex flex-col border-b p-6 lg:border-b-0 lg:border-r">
          <div className="flex flex-1 flex-col justify-center gap-3">
            <p className="text-sm text-muted-foreground">{t("Cache hit rate")}</p>
        ...
        <div className="flex flex-col gap-3 p-6">
          <div className="flex items-baseline justify-between">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("Share of turns")}</p>
            <p className="text-xs text-muted-foreground">
              <span className="text-lg font-semibold tabular-nums text-foreground">
                {total.toLocaleString()}
              </span>{" "}
              {t("{0} turns measured", total.toLocaleString())}
            </p>
          </div>
          ...
          {cache.unordered_turns > 0 && (
            <p className="text-xs text-muted-foreground">
              {t(
                "{0} turns arrived out of order across pods and are not bucketed",
                cache.unordered_turns.toLocaleString(),
              )}
            </p>
          )}
        </div>
...
const BenchmarksBody: React.FC<BenchmarksBodyProps> = ({ isPending, error, data, selectedKey }) => {
  if (isPending) return <Message>{t("Loading auto-router usage...")}</Message>;
  if (error instanceof ApiError && error.status === 403) {
    return <Message>{t("Auto-router usage is visible to proxy admin roles only")}</Message>;
  }
  if (error || !data) return <Message>{t("Auto-router usage is unavailable right now")}</Message>;
  if (data.groups.length === 0) return <Message>{t("No auto-router sessions in this window yet")}</Message>;
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Metric label={t("Avg turns per session")} value={stats.avg_turns_per_session.toFixed(1)} />
        <Metric label={t("Avg session length")} value={durationLabel(stats.avg_session_seconds)} />
        <Metric label={t("Avg tokens per session")} value={formatNumberWithCommas(stats.avg_tokens_per_session, 1, true)} />
      </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-baseline gap-2">
          <h3 className="text-lg font-semibold text-foreground">{t("Auto-router prompt caching")}</h3>
          <p className="text-xs text-muted-foreground">
            {t("every turn falls in exactly one bucket, by what the router did")}
          </p>
        </div>
...
const AutoRouterBenchmarksTab: React.FC<AutoRouterBenchmarksTabProps> = ({ accessToken }) => {
  ...
  const selectedLabel = data ? t(viewFor(data, selectedKey).label) : t("All auto-routers");

  return (
    <div className="w-full space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-foreground">{t("Auto-router usage")}</h2>
          <p className="mt-1 text-sm text-muted-foreground">{t(WINDOW_LABELS[range])}</p>
        </div>
        <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
          ...
          <div className="w-full sm:w-64">
            <Select value={selectedKey} onValueChange={(value: string | null) => setSelectedKey(value ?? ALL_ROUTERS)}>
              ...
              <SelectContent>
                <SelectItem value={ALL_ROUTERS}>{t("All auto-routers")}</SelectItem>
                {groups.map((g) => (
                  <SelectItem key={groupKey(g)} value={groupKey(g)}>
                    {t(groupLabel(g, groups))}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
