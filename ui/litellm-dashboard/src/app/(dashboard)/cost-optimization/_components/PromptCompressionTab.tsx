...
const PromptCompressionTab: React.FC<PromptCompressionTabProps> = ({ accessToken }) => {
  ...
  const loadGuardrails = useCallback(() => {
    ...
    getGuardrailsList(accessToken)
      ...
      .catch((error) => {
        ...
        NotificationsManager.fromBackend(t("Failed to load compression guardrails"));
      })
  ...
  const handleAdd = async (values: CompressionFormValues) => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("Failed to create compression guardrail"));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <Card>
        ...
        <CardContent>
          <p className="mb-4 text-sm text-muted-foreground">
            ...
            >
              {t("Headroom setup docs")}
            </a>
          </p>
          {isLoading && <p className="text-sm text-muted-foreground">{t("Loading...")}</p>}
          {!isLoading && guardrails.length === 0 && (
            <p className="text-sm text-muted-foreground">
              {t("No prompt compression guardrails configured yet. Add one below to start saving on input tokens")}
            </p>
          )}
          {!isLoading && guardrails.length > 0 && (
            <ul className="divide-y divide-gray-200">
              {guardrails.map((guardrail) => (
                <li key={guardrail.guardrail_id} className="flex items-center justify-between py-3">
                  ...
                  >
                    {guardrail.litellm_params?.default_on ? t("Always on") : t("Opt-in")}
                  </span>
                </li>
              ))}
            </ul>
      ...
      <Card>
        ...
        <CardContent>
          ...
          >
            ...
            >
              <Input placeholder={t("headroom-compression")} />
            </Form.Item>
            <Form.Item
              name="apiBase"
              ...
              extra={t("The URL where your Headroom compression service is hosted")}
              rules={[{ required: true, message: t("API base is required") }]}
            >
              <Input placeholder={t("https://your-headroom-endpoint")} />
            </Form.Item>
            <Form.Item name="defaultOn" label={t("Apply to all requests")} valuePropName="checked">
              <Switch />
            </Form.Item>
