...
const VectorStoreInfoView: React.FC<VectorStoreInfoViewProps> = ({
  ...
  const fetchVectorStoreDetails = async () => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("Error fetching vector store details: ") + error);
      setLoadFailed(true);
    }
  };
  ...
  const handleSave = async (values: any) => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("Error updating vector store: ") + error);
    }
  };
  ...
  if (loadFailed) {
    return (
      <div className="p-4 max-w-full">
        ...
        <Title>{t("Vector store not found")}</Title>
        <Text className="text-gray-500">
          {t("Vector store {0} could not be loaded. It may have been deleted.", vectorStoreId)}
        </Text>
      </div>
    );
  }
    ...
    <div className="p-4 max-w-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <Title>{t("Vector Store ID: {0}", vectorStoreDetails.vector_store_id)}</Title>
          <Text className="text-gray-500">
            {vectorStoreDetails.vector_store_description || t("No description")}
          </Text>
        </div>
