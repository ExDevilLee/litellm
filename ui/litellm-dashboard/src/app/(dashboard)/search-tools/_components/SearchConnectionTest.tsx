...
const SearchConnectionTest: React.FC<SearchConnectionTestProps> = ({ litellmParams, accessToken, onTestComplete }) => {
  ...
  const getCleanErrorMessage = (errorMsg: string) => {
    if (!errorMsg) return t("Unknown error");
    ...
    if (finalError.includes("<html>") || finalError.includes("<!DOCTYPE")) {
      ...
      if (finalError.includes("401") || finalError.includes("Authorization Required")) {
        return t("Authentication failed: Invalid API key or credentials");
      }
      return t("Authentication error - please check your API key");
    }
  ...
  const errorMessage = testResult?.message ? getCleanErrorMessage(testResult.message) : t("Unknown error");
      ...
      <div className="rounded-lg bg-card p-6">
        <div className="flex flex-col items-center justify-center px-5 py-8">
          ...
          <p className="text-base text-foreground">
            {t("Testing connection to {0}...", litellmParams.search_provider || t("search provider"))}
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-center justify-center px-5 py-8">
          ...
          <div className="ml-3">
            <p className="text-lg font-medium text-emerald-600">
              {t("Connection to {0} successful!", litellmParams.search_provider)}
            </p>
            ...
            {testResult.results_count !== undefined && (
              <p className="text-sm text-muted-foreground">
                {t("Results retrieved: {0}", testResult.results_count)}
              </p>
            )}
          </div>
        </div>
        ...
        <div>
          <div className="mb-5 flex items-center">
            ...
            <p className="text-lg font-medium text-destructive">
              {t("Connection to {0} failed", litellmParams.search_provider || t("search provider"))}
            </p>
          </div>
