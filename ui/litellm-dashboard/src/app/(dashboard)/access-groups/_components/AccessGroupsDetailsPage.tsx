        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 text-sm text-muted-foreground">
              <span>{t("ID: {0}", accessGroup.access_group_id)}</span>
            ...
            {keyIds.length > MAX_PREVIEW && (
              <CardAction>
                <Button variant="link" size="sm" onClick={() => setShowAllKeys(!showAllKeys)}>
                  {showAllKeys ? t("Show Less") : t("View All ({0})", keyIds.length)}
                </Button>
              </CardAction>
            )}
          </CardHeader>
            ...
            {teamIds.length > MAX_PREVIEW && (
              <CardAction>
                <Button variant="link" size="sm" onClick={() => setShowAllTeams(!showAllTeams)}>
                  {showAllTeams ? t("Show Less") : t("View All ({0})", teamIds.length)}
                </Button>
              </CardAction>
            )}
          </CardHeader>
        ...
        <CardContent>
          <Tabs defaultValue="models">
            ...
            <TabsContent value="models" className="pt-4">
              <ResourceList ids={modelIds} emptyMessage={t("No models assigned to this group")} />
            </TabsContent>
            <TabsContent value="mcp" className="pt-4">
              <ResourceList ids={mcpServerIds} emptyMessage={t("No MCP servers assigned to this group")} />
            </TabsContent>
            <TabsContent value="agents" className="pt-4">
              <ResourceList ids={agentIds} emptyMessage={t("No agents assigned to this group")} />
            </TabsContent>
          </Tabs>
        </CardContent>
      </Card>
