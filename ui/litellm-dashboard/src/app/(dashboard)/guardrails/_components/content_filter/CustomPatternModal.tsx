...
const CustomPatternModal: React.FC<CustomPatternModalProps> = ({
  ...
  return (
    <Modal title={t("Add custom regex pattern")} open={visible} onCancel={onCancel} footer={null} width={800}>
      <Space direction="vertical" style={{ width: "100%" }} size="large">
        <div>
          <Text strong>{t("Pattern name")}</Text>
          <Input
        ...
        <div>
          ...
          <Input
            placeholder={t("e.g., ID-[0-9]{6}")}
            value={patternRegex}
            onChange={(e) => onRegexChange(e.target.value)}
            style={{ marginTop: 8 }}
          />
          <Text type="secondary" style={{ fontSize: 12 }}>
            {t("Enter a valid regular expression to match sensitive data")}
          </Text>
        </div>
