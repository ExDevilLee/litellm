...
const LLMJudgeFields: React.FC<LLMJudgeFieldsProps> = ({ availableModels, form }) => {
  return (
    <>
      ...
      >
        {t("After each LLM response, the")} <strong>{t("Judge Model")}</strong>{" "}
        {t(
          "scores it 0–100 against your criteria. If the weighted average falls below the threshold, the response is blocked (or logged).",
        )}
      </div>
      ...
      <Form.Item
        ...
        rules={[{ required: true, message: t("Select a judge model") }]}
      >
        <Select
          showSearch
      ...
      >
        <Select>
          <Select.Option value="block">{t("Block (return 422)")}</Select.Option>
          <Select.Option value="log">{t("Log only")}</Select.Option>
        </Select>
      </Form.Item>
      ...
      <Form.Item
        label={
          <span>
            ...
            <Tooltip title={t("Each criterion is something the judge checks. Weights must add up to 100%.")}>
              <QuestionCircleOutlined style={{ color: "#8c8c8c" }} />
            </Tooltip>
          </span>
        }
                ...
                >
                  <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
                    <Form.Item
                      {...restField}
                      name={[name, "name"]}
                      rules={[{ required: true, message: t("Enter criterion name") }]}
                      style={{ flex: 2, marginBottom: 8 }}
                    >
                      <Input placeholder={t("Criterion name (e.g. Policy accuracy)")} />
                    </Form.Item>
                    <Form.Item
                      {...restField}
                      name={[name, "weight"]}
                      ...
                      rules={[{ required: true, message: t("Enter weight") }]}
                  ...
                  <Form.Item
                    ...
                    rules={[{ required: true, message: t("Describe what to check") }]}
                    style={{ marginBottom: 8 }}
                  >
                    <Input placeholder={t("What should the judge check for this criterion?")} />
                  </Form.Item>
                </div>
              ))}
              <Button
                  ...
                  {() => {
                    ...
                    return (
                      <div style={{ marginTop: 6, fontSize: 12, color: weightOk ? "#52c41a" : "#faad14" }}>
                        {t("Weights total: {0}%", weightTotal)}
                        {weightOk ? " ✓" : t("— must add up to 100%")}
                      </div>
                    );
                  }}
                </Form.Item>
