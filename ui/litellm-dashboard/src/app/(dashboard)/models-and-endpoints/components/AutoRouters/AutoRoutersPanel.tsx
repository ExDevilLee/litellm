...
export function AutoRoutersPanel({ accessToken, userRole, userID, teams, createScope }: AutoRoutersPanelProps) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("Deleted auto router: {0}", deletingRouter.name));
      setDeletingRouter(null);
      await invalidateAutoRouters();
    } catch (error) {
      NotificationsManager.fromBackend(t("Failed to delete auto router: {0}", String(error)));
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="w-full space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {t("Auto routers sit above your deployments and pick a model per request. They are called like any other model, so clients keep using a single model name.")}
          </p>
        </div>
        {canCreate && (
      ...
      <Dialog open={isCreating} onOpenChange={setIsCreating}>
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("Routes each request to a model by classifying its complexity. Called like any other model, so clients keep using a single model name.")}
            </DialogDescription>
          </DialogHeader>
          <AddAutoRouterTab
      ...
      {deletingRouter && (
        <DeleteResourceModal
          ...
          message={t('Are you sure you want to delete "{0}"? Any client still calling this model name will start failing.', deletingRouter.name)}
          resourceInformationTitle={t("Auto router")}
          resourceInformation={[
            { label: t("Name"), value: deletingRouter.name },
            { label: t("Type"), value: t(deletingRouter.typeLabel) },
            { label: t("ID"), value: deletingRouter.id },
          ]}
