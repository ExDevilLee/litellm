    ...
    >
      ...
      <Row gutter={24}>
        <Col span={12}>
          ...
          >
            <Input placeholder={t("e.g. Customer Support Bot")} />
          </Form.Item>
        </Col>
        <Col span={12}>
          <Form.Item name="team_id" label={t("Team")} rules={[{ required: true, message: t("Please select a team") }]}>
            <Select
              showSearch
      ...
      <Row>
        <Col span={24}>
          <Form.Item name="description" label={t("Description")}>
            <Input.TextArea placeholder={t("Describe the purpose of this project")} rows={3} />
          </Form.Item>
        </Col>
      </Row>
      ...
      <Row>
        <Col span={24}>
          <Form.Item
            name="models"
            label={t("Allowed Models (scoped to selected team's models)")}
            help={!selectedTeam ? t("Select a team first to see available models") : undefined}
          >
            <Select
              mode="multiple"
              placeholder={selectedTeam ? t("Select models") : t("Select a team first")}
                ...
                children: (
                  <>
                    <Flex align="center" gap={12}>
                      <Typography.Text strong>{t("Block Project")}</Typography.Text>
                      ...
                      {({ getFieldValue }) =>
                        getFieldValue("isBlocked") ? (
                          <Alert
                            ...
                            type="warning"
                            showIcon
                            message={t("All API requests using keys under this project will be rejected.")}
                            style={{ marginTop: 12 }}
                          />
                        ) : null
                      }
                          ...
                          {fields.map(({ key, name, ...restField }) => (
                            <Space key={key} style={{ display: "flex", marginBottom: 8 }} align="baseline">
                              <Form.Item
                                ...
                                name={[name, "model"]}
                                rules={[
                                  { required: true, message: t("Missing model") },
                                  {
                                    validator: (_, value) => {
                                      ...
                                      if (dupes.length > 1) {
                                        return Promise.reject(new Error(t("Duplicate model")));
                                      }
                              ...
                              >
                                <Input placeholder={t("Model name (e.g. gpt-4)")} />
                              </Form.Item>
                              <Form.Item {...restField} name={[name, "tpm"]}>
                                <InputNumber placeholder={t("TPM Limit")} min={0} />
                              </Form.Item>
                              <Form.Item {...restField} name={[name, "rpm"]}>
                                <InputNumber placeholder={t("RPM Limit")} min={0} />
                              </Form.Item>
                          ...
                          {fields.map(({ key, name, ...restField }) => (
                            <Space key={key} style={{ display: "flex", marginBottom: 8 }} align="baseline">
                              <Form.Item
                                ...
                                name={[name, "key"]}
                                rules={[
                                  { required: true, message: t("Missing key") },
                                  {
                                    validator: (_, value) => {
                                      if (!value) return Promise.resolve();
                                      ...
                                      if (dupes.length > 1) {
                                        return Promise.reject(new Error(t("Duplicate key")));
                                      }
                                      return Promise.resolve();
                                    },
                                  },
