    ...
    try {
      await usageAiChatStream(
        ...
        (errorMsg: string) => {
          setStatusMessage(null);
          setActiveToolCalls([]);
          setMessages((prev) => [...prev, { role: "assistant", content: t("Error: {0}", errorMsg) }]);
          setStreamingContent("");
        },
        (status: string) => {
      ...
      <div className="px-5 py-3 border-b border-gray-100 shrink-0">
        ...
        >
          <ComboboxInput
            ...
            aria-label={t("Select a model (optional, defaults to gpt-4o-mini)")}
            aria-busy={isLoadingModels}
            showClear={selectedModel !== undefined}
          />
          <ComboboxContent>
            <ComboboxEmpty>{isLoadingModels ? t("Loading models…") : t("No models found")}</ComboboxEmpty>
            <ComboboxList>
              {(model: string) => (
