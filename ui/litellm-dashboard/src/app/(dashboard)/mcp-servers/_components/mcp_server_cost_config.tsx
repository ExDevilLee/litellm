      ...
      <Card className="p-6">
        <div className="space-y-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-lg font-medium">{t("Cost Configuration")}</h3>
            <Tooltip>
              <TooltipTrigger
                render={<Info className="size-4 text-muted-foreground" aria-label="About cost configuration" />}
              />
              <TooltipContent>
                {t("Configure costs for this MCP server's tool calls. Set a default rate and per-tool overrides.")}
              </TooltipContent>
            </Tooltip>
          </div>
            ...
            <div>
              <label className="mb-2 block text-sm font-medium">
                ...
                <Tooltip>
                  ...
                  />
                  <TooltipContent>{t("Default cost charged for each tool call to this server.")}</TooltipContent>
                </Tooltip>
              </label>
              <CostInput
              ...
              <div className="space-y-4">
                <label className="block text-sm font-medium">
                  ...
                  <Tooltip>
                    ...
                    />
                    <TooltipContent>
                      {t("Override the default cost for specific tools. Leave blank to use the default rate.")}
                    </TooltipContent>
                  </Tooltip>
                </label>
              ...
              <div className="mt-2 space-y-1">
                {value.default_cost_per_query && (
                  <p className="text-sm text-muted-foreground">
                    {t("• Default cost: ${0} per query", value.default_cost_per_query.toFixed(4))}
                  </p>
                )}
                {value.tool_name_to_cost_per_query &&
                    ...
                    ([toolName, cost]) =>
                      ...
                      cost !== undefined && (
                        <p key={toolName} className="text-sm text-muted-foreground">
                          {t("• {0}: ${1} per query", toolName, cost.toFixed(4))}
                        </p>
                      ),
                  )}
              </div>
