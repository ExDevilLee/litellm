    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden">
      ...
      >
        <span className="text-sm font-medium text-gray-700 flex items-center gap-2">
          ...
          {serverName}
          {selectedSet.size > 0 && (
            <span className="ml-1 text-xs text-purple-600 font-semibold">{t("{0} selected", selectedSet.size)}</span>
          )}
        </span>
      ...
      {expanded && (
        <div className="p-2">
          {loading ? (
            ...
            <p className="text-xs text-gray-400 px-2 py-2">{t("No tools found for this server.")}</p>
          ) : (
...
function CreateToolsetModal({ open, onClose, onSave, accessToken, initialToolset }: CreateToolsetModalProps) {
  ...
  return (
    <Modal
      open={open}
      onCancel={onClose}
      title={initialToolset ? t("Edit Toolset") : t("New Toolset")}
      width={960}
      footer={null}
      forceRender
    >
      <Form form={form} layout="vertical" className="mt-2">
        <div className="flex gap-4 mb-4">
          <Form.Item
            label={t("Toolset Name")}
            name="toolset_name"
            rules={[{ required: true, message: t("Please enter a toolset name") }]}
            className="flex-1 mb-0"
          >
            <Input placeholder={t("e.g. github-linear-tools")} />
          </Form.Item>
          <Form.Item label={t("Description")} name="description" className="flex-1 mb-0">
            <Input placeholder={t("Optional description")} />
          </Form.Item>
        </div>
      </Form>
      ...
      <div className="flex gap-4 mt-2" style={{ minHeight: 360 }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2">
            <Text className="text-sm font-semibold text-gray-700">{t("Available Tools")}</Text>
          </div>
          <Input
          ...
          <div className="space-y-2 overflow-y-auto" style={{ maxHeight: 300 }}>
            {filteredServers.length === 0 ? (
              <Text className="text-gray-400 text-sm">
                {mcpServers.length === 0 ? t("No MCP servers configured") : t("No servers match your search")}
              </Text>
            ) : (
        ...
        <div className="w-72 shrink-0">
          <Text className="text-sm font-semibold text-gray-700 mb-2 block">
            {t("Your Toolset")} <span className="text-xs font-normal text-gray-400">({selectedTools.length} {t("tools")})</span>
          </Text>
          <div className="space-y-1 overflow-y-auto" style={{ maxHeight: 340 }}>
            {selectedTools.length === 0 ? (
              <Text className="text-gray-400 text-sm">{t("No tools added yet")}</Text>
            ) : (
      ...
      <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-gray-200">
        ...
        <Button onClick={handleSubmit} loading={saving}>
          {initialToolset ? t("Save Changes") : t("Create Toolset")}
        </Button>
      </div>
    </Modal>
  );
...
function ToolsetsEmptyState() {
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("No toolsets yet")}</div>
      <div className="text-sm text-muted-foreground">
        {t("Create a toolset to give keys and teams a curated set of MCP tools.")}
      </div>
    </div>
  );
}
...
function ToolsetUsageGuide() {
  ...
  return (
    <div className="mb-6 rounded-lg border border-gray-200 bg-gray-50 px-5 py-4">
      <p className="text-sm font-medium text-gray-700 mb-1">{t("How toolsets work")}</p>
      <p className="text-sm text-gray-500 mb-3">
        {t("Create a toolset, assign it to a key via")}{" "}
        <span className="font-medium text-gray-700">{t("API Keys → Edit Key → MCP Servers")}</span>
        {t(", then point your MCP client at the toolset URL. The client only sees the tools you picked.")}
      </p>
      <div className="text-xs text-gray-400 mb-1">{t("Claude Code / Cursor config")}</div>
...
export function MCPToolsetsTab({ accessToken, userRole }: MCPToolsetsTabProps) {
  ...
  const handleCreate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    message.success(t("Toolset created"));
  ...
  const handleUpdate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    message.success(t("Toolset updated"));
    queryClient.invalidateQueries({ queryKey: ["mcpToolsets"] });
    setEditToolset(null);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      message.success(t("Toolset deleted"));
      queryClient.invalidateQueries({ queryKey: ["mcpToolsets"] });
      setDeleteId(null);
    } finally {
      setDeleting(false);
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <Title>{t("MCP Toolsets")}</Title>
          <Text className="text-gray-500 text-sm">
            {t("Curated collections of tools from one or more MCP servers. Assign toolsets to keys and teams via the MCP permissions dropdown.")}
          </Text>
        </div>
        {isAdmin && (
      ...
      <DataTable
        ...
        isLoading={isLoading}
        loadingMessage={t("Loading toolsets…")}
      ...
      <Modal
        ...
        title={t("Delete Toolset")}
      >
        <p>
          {t("Are you sure you want to delete this toolset? Keys and teams using it will lose access to the scoped tools.")}
        </p>
      </Modal>
    </div>
  );
