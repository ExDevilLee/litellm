...
const MCPServers: React.FC<MCPServerProps> = ({ accessToken, userRole, userID }) => {
  ...
  const teamSelectItems = React.useMemo(
    () => ({
      all: isInternalUser ? t("All Available Servers") : t("All Servers"),
      personal: t("Personal"),
          ...
          <AlertDialogContent>
            ...
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                {t("This action is permanent and cannot be undone. All associated configurations will be removed.")}
              </p>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel disabled={isDeletingServer}>{t("Cancel")}</AlertDialogCancel>
              <Button variant="destructive" disabled={isDeletingServer} onClick={confirmDelete}>
                {isDeletingServer ? t("Deleting...") : t("Delete")}
              </Button>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-semibold">{t("MCP Servers")}</h1>
              {filteredServers.length > 0 && <Badge variant="secondary">{filteredServers.length}</Badge>}
            </div>
            <p className="mt-1 text-sm text-muted-foreground">{t("Configure and manage your MCP servers")}</p>
          </div>
              ...
              <div className="w-full h-full">
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="all">
                              {isInternalUser ? t("All Available Servers") : t("All Servers")}
                            </SelectItem>
                      ...
                      <div className="flex items-center gap-2">
                        <p className="flex items-center text-sm font-medium whitespace-nowrap text-muted-foreground">
                          ...
                          <Tooltip>
                            ...
                            />
                            <TooltipContent>
                              {t("An MCP Access Group is a set of users or teams that have permission to access specific MCP servers. Use access groups to control and organize who can connect to which servers.")}
                            </TooltipContent>
                          </Tooltip>
                        </p>
                        <Select
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="all">{t("All Access Groups")}</SelectItem>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  <InputGroup className="max-w-80">
                    ...
                    <InputGroupInput
                      placeholder={t("Search by name, alias, URL, or ID")}
                      value={searchQuery}
                  ...
                  <div className="ml-auto text-xs text-muted-foreground">
                    {t("{0} of {1} servers", displayedServers.length, filteredServers.length)}
                  </div>
                </div>
                <div className="mt-4 w-full">
                  {isLoadingServers ? (
                    <div className="flex items-center justify-center gap-3 rounded-lg border border-dashed border-border bg-card p-12">
                      ...
                      <p className="text-sm text-muted-foreground">{t("Loading MCP servers...")}</p>
                    </div>
                  ) : displayedServers.length === 0 ? (
                    <div className="rounded-lg border border-dashed border-border bg-card p-12 text-center">
                      <p className="text-sm text-muted-foreground">
                        {filteredServers.length === 0
                          ? t("No MCP servers configured. Click '+ Add New MCP Server' to get started.")
                          : t("No servers match the current filters or search.")}
                      </p>
                    </div>
                  ) : (
                    <div
