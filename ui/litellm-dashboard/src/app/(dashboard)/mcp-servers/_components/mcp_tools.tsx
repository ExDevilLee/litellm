  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (result?.error) {
        ...
        const enhancedError = new Error(result.message || result.error || t("Failed to fetch MCP tools")) as Error & {
          status?: number;
          statusText?: string;
          details?: any;
        };
      ...
      <Card className="w-full overflow-hidden rounded-xl shadow-md">
        <div className="grid h-auto w-full grid-cols-4 gap-4">
          ...
          <div className="col-span-1 flex flex-col bg-muted p-4">
            <h2 className="mt-2 mb-6 text-xl font-semibold">{t("MCP Tools")}</h2>
            ...
            <div className="flex flex-col flex-1">
                ...
                <div className="mb-4 rounded-lg border border-border bg-card p-3">
                  <div className="mb-2 flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <p className="text-sm font-medium">{t("Additional Headers")}</p>
                    </div>
                    <Button variant="link" size="sm" onClick={() => setShowHeaderInput(!showHeaderInput)}>
                      {showHeaderInput ? t("Hide") : t("Configure")}
                    </Button>
                  </div>

                  {!showHeaderInput && Object.keys(passthroughHeaders).length === 0 && (
                    <p className="text-xs text-muted-foreground">
                      {t("This server requires additional headers. Click \"Configure\" to provide values.")}
                    </p>
                  )}
                      ...
                      {extraHeaders?.map((headerName) => (
                        <div key={headerName}>
                          ...
                          <InputGroup className="w-full">
                            ...
                            <InputGroupInput
                              placeholder={t("Enter {0}", headerName)}
                  ...
                  {!showHeaderInput && Object.keys(passthroughHeaders).length > 0 && (
                    <div className="mt-2">
                      <p className="flex items-center text-xs text-muted-foreground">
                        ...
                        {t("{0} header(s) configured", Object.keys(passthroughHeaders).length)}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col flex-1 min-h-0">
                <p className="mb-3 flex items-center text-sm font-medium">
                  <Wrench className="mr-2 size-4" /> {t("Available Tools")}
                  {toolsData.length > 0 && (
                ...
                {usesBrowserHeldToken && !oauthToken && (
                  <div className="rounded-lg border border-border bg-card p-4 text-center">
                    ...
                    <p className="mb-1 text-xs font-medium">{t("Authentication required")}</p>
                    <p className="mb-3 text-xs text-muted-foreground">{t("Authenticate to view available tools")}</p>
                    <Button
                      size="sm"
                ...
                {(authorizationCodeNeedsAuth || authorizationCodeTokenRejected) && (
                  <div className="rounded-lg border border-border bg-card p-4 text-center">
                    ...
                    <p className="mb-3 text-xs text-muted-foreground">
                      {t("Authenticate with the upstream provider to view available tools")}
                    </p>
                    <Button
                      size="sm"
                  ...
                  <>
                    ...
                    {toolsAreaLoading && (
                      <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card py-8">
                        ...
                        <p className="text-xs font-medium">{t("Loading tools...")}</p>
                      </div>
                    )}
                    ...
                    {(mcpToolsResponse?.error || mcpToolsError) && !toolsAreaLoading && !toolsData.length && (
                      <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-xs text-destructive">
                        <p className="font-medium">
                          {t("Error: {0}", mcpToolsResponse?.message || (mcpToolsError as Error)?.message)}
                        </p>
                      </div>
                    )}
                    ...
                    {!toolsAreaLoading &&
                      ...
                      (!toolsData || toolsData.length === 0) && (
                        <div className="rounded-lg border border-border bg-card p-4 text-center">
                          ...
                          <p className="mb-1 text-xs font-medium">{t("No tools available")}</p>
                          <p className="text-xs text-muted-foreground">{t("No tools found for this server")}</p>
                        </div>
                      )}
                      ...
                      <>
                        {filteredTools.length === 0 ? (
                          <div className="rounded-lg border border-border bg-card p-4 text-center">
                            ...
                            <p className="mb-1 text-xs font-medium">{t("No tools found")}</p>
                            <p className="text-xs text-muted-foreground">{t("No tools match \"{0}\"", toolSearchTerm)}</p>
                          </div>
                        ) : (
          ...
          <div className="col-span-3 flex flex-col">
            <div className="flex items-center justify-between border-b border-border p-4">
              <h2 className="mb-0 text-xl font-semibold">{t("Tool Testing Playground")}</h2>
            </div>
            ...
            <div className="flex-1 overflow-auto p-4">
              {!selectedTool ? (
                ...
                <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
                  ...
                  <p className="mb-2 text-lg font-medium">{t("Select a Tool to Test")}</p>
                  <p className="max-w-md text-center text-sm">
                    {t("Choose a tool from the left sidebar to start testing its functionality with custom inputs.")}
                  </p>
                </div>
              ) : (
                /* Tool Test Panel */
