...
const ChatUI: React.FC<ChatUIProps> = ({
  ...
  const handleSendMessage = async () => {
    ...
    if (endpointType === EndpointType.MCP) {
      ...
      if (!rawSelected) {
        NotificationsManager.fromBackend(t("Please select an MCP server to test"));
        return;
      }
      if (!selectedMCPDirectTool) {
        NotificationsManager.fromBackend(t("Please select an MCP tool to call"));
        return;
      }
      ...
      if (!mcpTool) {
        NotificationsManager.fromBackend(t("Please wait for tool schema to load"));
        return;
      }
      try {
        ...
        NotificationsManager.fromBackend(
          err instanceof Error ? err.message : t("Please fill in all required parameters"),
        );
        return;
      }
    }
    ...
    if (endpointType === EndpointType.RESPONSES && responsesUploadedImage) {
      ...
      const audioMessage = inputMessage
        ? t("🎵 Audio file: {0}\nPrompt: {1}", uploadedAudio.name, inputMessage)
        : t("🎵 Audio file: {0}", uploadedAudio.name);
      ...
      const mcpMessage = t(
        ...
        JSON.stringify(mcpToolArguments, null, 2),
      );
    ...
    try {
      ...
      if (endpointType === EndpointType.MCP) {
        ...
        if (resolvedServerId && !resolvedServerId.startsWith("toolset:") && selectedMCPDirectTool) {
          ...
          updateTextUI("assistant", resultText || t("Tool executed successfully."));
        }
      }
      ...
      if (signal.aborted) {
        ...
        updateTextUI("assistant", t("Error fetching response: {0}", error instanceof Error ? error.message : String(error)));
      }
    } finally {
      setIsLoading(false);
      abortControllerRef.current = null;
  ...
  let modelEmptyText = t("No models available for this key");
  if (modelLoadError) {
    modelEmptyText = t("Unable to load models for this key");
  } else if (apiKeySource === "custom" && !apiKey.trim()) {
    modelEmptyText = t("Enter a Virtual Key to load models");
  } else if (modelInfo.length > 0 && modelsForEndpoint.length === 0) {
    modelEmptyText = t("No models available for this endpoint");
  }
            ...
            <div className="max-h-[42%] w-full shrink-0 overflow-y-auto border-b border-gray-200 bg-gray-50 p-4 lg:max-h-none lg:w-72 lg:border-r lg:border-b-0 xl:w-80">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {customProxyBaseUrl && (
                    <p className="mt-1 text-xs text-gray-500">
                      {t("API calls will be sent to: {0}", customProxyBaseUrl)}
                    </p>
                  )}
                </div>
                  ...
                  {endpointType === EndpointType.SPEECH && (
                    <div className="mb-4">
                      ...
                      <ShadcnSelect value={selectedVoice} onValueChange={handleVoiceChange}>
                        <SelectTrigger className="w-full" size="sm" aria-label={t("Voice")}>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <SearchSelect
                      ...
                      options={[
                        ...
                        ...modelsForEndpoint.map((model) => ({
                          ...
                          sublabel: model.mode ? t("Mode: {0}", model.mode) : undefined,
                        })),
                      ]}
                    />
                ...
                {endpointType === EndpointType.A2A_AGENTS && (
                  <div>
                    <label className="mb-2 flex items-center text-sm font-medium text-gray-700">
                      <Bot className="mr-2 size-4" aria-hidden="true" /> {t("Select Agent")}
                    </label>
                    <SearchSelect
                      value={selectedAgent}
                ...
                <div>
                  ...
                  {endpointType === EndpointType.MCP &&
                    ...
                    (() => {
                      ...
                      return (
                        <div className="mt-3">
                          <p className="mb-1 block text-xs text-gray-600">{t("Select Tool")}</p>
                          <SearchSelect
                          ...
                          return (
                            ...
                            >
                              <p className="text-xs text-blue-700">
                                {t("{0} requires your API key", serverName)}
                              </p>
                ...
                <div className="min-h-0 min-w-0 flex-1 overflow-auto p-3 pb-0 sm:p-4 sm:pb-0">
                  ...
                  {isLoading && (
                    <div className="my-4 flex items-center justify-center">
                      <Loader2 className="size-6 animate-spin text-gray-500" aria-label={t("Loading")} />
                    </div>
                  )}
                    ...
                    <div className="mb-4">
                      {uploadedImages.length === 0 ? (
                        ...
                        >
                          ...
                          <p className="text-sm">{t("Click or drag images to upload")}</p>
                          <p className="text-xs text-gray-500">
                            {t("Support for PNG, JPG, JPEG, GIF, WebP. Multiple images supported.")}
                          </p>
                          <input
                            type="file"
                        ...
                        <div className="flex flex-wrap gap-2">
                          {uploadedImages.map((file, index) => (
                            <div key={index} className="relative inline-block">
                              <img
                                ...
                                alt={t("Upload preview {0}", index + 1)}
                          ...
                          <label className="flex h-32 w-32 cursor-pointer flex-col items-center justify-center rounded-md border-2 border-dashed border-gray-300 hover:border-gray-400">
                            ...
                            <p className="mt-1 text-xs text-gray-500">{t("Add more")}</p>
                            <input
                              type="file"
                              accept={IMAGE_EDIT_ACCEPT}
                              multiple
                    ...
                    <div className="mb-4">
                      {!uploadedAudio ? (
                        ...
                        >
                          ...
                          <p className="text-sm">{t("Click or drag audio file to upload")}</p>
                          <p className="text-xs text-gray-500">
                        ...
                        <div className="flex items-center gap-2">
                          {isLoading ? (
                            <>
                              ...
                              <span className="text-sm font-medium text-blue-700">{t("Running Python code...")}</span>
                            </>
                          ) : (
                            <>
                              <Code2 className="size-4 text-blue-500" aria-hidden="true" />
                              <span className="text-sm font-medium text-blue-700">{t("Code Interpreter Active")}</span>
                            </>
                          )}
                        </div>
                        <button
      ...
      <Dialog open={isGetCodeModalVisible} onOpenChange={setIsGetCodeModalVisible}>
        <DialogContent className="sm:max-w-3xl">
          ...
          <div className="my-2 flex items-end justify-between gap-3">
            <div>
              <p className="mb-1 text-sm font-medium text-gray-700">{t("SDK Type")}</p>
              <ShadcnSelect value={selectedSdk} onValueChange={(value) => setSelectedSdk(value as "openai" | "azure")}>
                <SelectTrigger className="w-[150px]" size="sm" aria-label={t("SDK Type")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="openai">{t("OpenAI SDK")}</SelectItem>
                  <SelectItem value="azure">{t("Azure SDK")}</SelectItem>
                </SelectContent>
              </ShadcnSelect>
            </div>
            <Button
              ...
              onClick={() => {
                void navigator.clipboard.writeText(generatedCode).then(
                  ...
                  () => NotificationsManager.error(t("Unable to copy to clipboard")),
                );
              }}
            >
              {t("Copy to Clipboard")}
      ...
      <Dialog open={isToolsetsInfoModalVisible} onOpenChange={setIsToolsetsInfoModalVisible}>
        <DialogContent className="sm:max-w-xl">
          ...
          <div className="space-y-4 py-2">
            ...
            <div>
              <h4 className="mb-2 font-semibold text-gray-800">{t("How to use a toolset:")}</h4>
              <ol className="list-inside list-decimal space-y-2 text-gray-700">
                <li>
                  {t("Select a")}{" "}
                  <span className="font-semibold text-violet-600">{t("Toolset")}</span>{" "}
                  {t("(purple badge) from the MCP Servers dropdown.")}
                </li>
            ...
            <div className="rounded-sm border border-purple-200 bg-purple-50 p-3">
              <p className="text-sm text-purple-800">
                ...
                {t(
                  'A "GitHub Read-only" toolset might include only',
                )}{" "}
                <code>list_repos</code> {t("and")} <code>get_file</code>{" "}
                {t("from a GitHub MCP server, preventing agents from making writes.")}
              </p>
            </div>
            <div>
              <h4 className="mb-1 font-semibold text-gray-800">{t("Creating toolsets:")}</h4>
              <p className="text-sm text-gray-600">
                {t("Admins can create and manage toolsets from the")} <strong>{t("MCP")}</strong> {t("page →")}{" "}
                <strong>{t("Toolsets")}</strong> {t("tab. Toolsets can then be assigned to keys and teams to scope their tool access.")}
              </p>
            </div>
          </div>
          <DialogFooter>
