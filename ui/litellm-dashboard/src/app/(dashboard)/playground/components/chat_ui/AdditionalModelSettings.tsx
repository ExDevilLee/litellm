    ...
    <div className="space-y-4 p-4 w-80">
      {onStreamingChange && (
        <div className="flex items-center gap-1">
          ...
          <Tooltip title={t("Streams the answer token by token. Uncheck to send a non-streaming request and render the full response at once.")}>
            <InfoCircleOutlined
              className="text-xs text-gray-400 cursor-pointer shrink-0 hover:text-gray-600"
              aria-label={t("Help: Stream responses")}
            />
          </Tooltip>
        </div>
      )}
      ...
      {onMockTestFallbacksChange && (
        <div className="flex items-center gap-1">
          <Checkbox checked={mockTestFallbacks ?? false} onChange={(e) => onMockTestFallbacksChange(e.target.checked)}>
            <span className="font-medium">{t("Simulate failure to test fallbacks")}</span>
          </Checkbox>
          <Popover
            trigger="hover"
            placement="right"
          ...
          >
            <InfoCircleOutlined
              className="text-xs text-gray-400 cursor-pointer shrink-0 hover:text-gray-600"
              aria-label={t("Help: Simulate failure to test fallbacks")}
            />
          </Popover>
        </div>
      )}
