  ...
  return (
    <div className="a2a-metrics mt-3 pt-2 border-t border-gray-200 text-xs">
      ...
      <div className="flex items-center mb-2 text-gray-600">
        ...
        <span className="font-medium text-gray-700">{t("A2A Metadata")}</span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 text-gray-500 ml-4">
        ...
        {totalLatency !== undefined && (
          <Tooltip>
            ...
            <TooltipContent>{t("Total latency")}</TooltipContent>
          </Tooltip>
        )}
        ...
        {timeToFirstToken !== undefined && (
          <Tooltip>
            ...
            <TooltipContent>{t("Time to first token")}</TooltipContent>
          </Tooltip>
        )}
      </div>
            ...
            <TooltipTrigger
              render={
                <Button
                  ...
                  aria-label={t("Copy task ID {0}", taskId)}
                />
              }
            >
              <FileText className="size-3" />
              {t("Task: {0}", truncateId(taskId))}
              <Copy className="size-3 text-gray-400" />
            </TooltipTrigger>
            <TooltipContent>{t("Click to copy: {0}", taskId)}</TooltipContent>
          </Tooltip>
        )}
            ...
            <TooltipTrigger
              render={
                <Button
                  ...
                  aria-label={t("Copy session ID {0}", contextId)}
                />
              }
            >
              <Link className="size-3" />
              {t("Session: {0}", truncateId(contextId))}
              <Copy className="size-3 text-gray-400" />
            </TooltipTrigger>
            <TooltipContent>{t("Click to copy: {0}", contextId)}</TooltipContent>
          </Tooltip>
        )}
      ...
      <Collapsible open={showDetails} onOpenChange={setShowDetails}>
        <CollapsibleContent>
          <div className="mt-2 ml-4 p-3 bg-gray-50 rounded-md text-gray-600 border border-gray-200">
            ...
            {status?.message && (
              <div className="mb-2">
                <span className="font-medium text-gray-700">{t("Status Message:")}</span>
            ...
            {taskId && (
              <div className="mb-1.5 flex items-center">
                <span className="font-medium text-gray-700 w-24">{t("Task ID:")}</span>
            ...
            {contextId && (
              <div className="mb-1.5 flex items-center">
                <span className="font-medium text-gray-700 w-24">{t("Session ID:")}</span>
                <code className="ml-2 px-2 py-1 bg-white border border-gray-200 rounded-sm text-xs font-mono">
                  {contextId}
                </code>
                <Button
            ...
            {metadata && Object.keys(metadata).length > 0 && (
              <div className="mt-3">
                <span className="font-medium text-gray-700">{t("Custom Metadata:")}</span>
