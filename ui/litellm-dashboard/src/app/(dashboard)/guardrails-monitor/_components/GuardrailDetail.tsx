...
export function GuardrailDetail({ guardrailId, onBack, accessToken = null, startDate, endDate }: GuardrailDetailProps) {
  ...
  if (detailLoading && !detailData) {
    return (
      <div role="status" aria-busy="true" aria-label={t("Loading")} className="flex items-center justify-center py-12">
          ...
          <div>
            <div className="mb-1 flex items-center gap-3">
              ...
              <StatusBadge
                ...
                label={t(data.status.charAt(0).toUpperCase() + data.status.slice(1))}
              />
            </div>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as string)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-6">
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
            <MetricCard label={t("Requests Evaluated")} value={data.requestsEvaluated.toLocaleString()} />
            <MetricCard
              label={t("Fail Rate")}
              value={`${data.failRate}%`}
              valueColor={data.failRate > 15 ? "text-red-600" : data.failRate > 5 ? "text-amber-600" : "text-green-600"}
              subtitle={t("{0} blocked", Math.round((data.requestsEvaluated * data.failRate) / 100).toLocaleString())}
            ...
            <MetricCard
              ...
              subtitle={data.avgLatency != null ? t("Per request (avg)") : t("No data")}
            />
          </div>
