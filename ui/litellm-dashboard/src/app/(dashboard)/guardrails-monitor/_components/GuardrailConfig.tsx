  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Button type="link" size="small" onClick={() => setShowVersionHistory(!showVersionHistory)}>
              {showVersionHistory ? t("Hide history") : t("View history")}
            </Button>
          </div>
          <div className="flex items-center gap-2">
            <Button icon={<RollbackOutlined />}>{t("Revert")}</Button>
            <Button type="primary" icon={<SaveOutlined />}>
              {t("Save as v{0}", parseInt(version.replace("v", ""), 10) + 1)}
            </Button>
          </div>
        </div>
            ...
            {versions.map((v) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-700">{t(v.changes)}</span>
                </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <p className="text-xs text-gray-500 mb-5">{t("Configure {0} behavior", guardrailName)}</p>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-semibold text-gray-900 flex items-center gap-2">
              ...
              {t("Custom Code Override")}
            </h3>
            <p className="text-xs text-gray-500 mt-0.5">
              {t("Replace the built-in guardrail with custom evaluation code")}
            </p>
          </div>
    ...
    return {"score": 0.9, "passed": True, "reason": "No violations"}`}
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <p className="text-xs text-gray-500 mb-4">
          {t("Re-run this guardrail on recent failing logs to validate your changes")}
        </p>
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            onClick={handleRerun}
          >
            {rerunStatus === "running" ? t("Running on 10 samples...") : t("Re-run on failing logs")}
          </Button>

          {rerunStatus === "success" && (
            <span className="text-sm text-green-600 flex items-center gap-2">
              <CheckCircleOutlined /> {t("7/10 would now pass with new config")}
            </span>
          )}
