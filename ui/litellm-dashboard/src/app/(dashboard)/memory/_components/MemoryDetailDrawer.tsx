        ...
        {row && (
          <div className="flex flex-col gap-4 px-4 pb-4">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              <span>
                {t("Created {0}", formatTimestamp(row.created_at))}
                {row.created_by ? ` ${t("by {0}", row.created_by)}` : ""}
              </span>
              <span aria-hidden="true">·</span>
              <span>
                {t("Updated {0}", formatTimestamp(row.updated_at))}
                {row.updated_by ? ` ${t("by {0}", row.updated_by)}` : ""}
              </span>
            </div>
          </div>
        )}
