...
const AddAgentForm: React.FC<AddAgentFormProps> = ({ visible, onClose, accessToken, onSuccess, teams }) => {
  ...
  const handleCreateAgent = async () => {
    ...
    try {
      ...
      if (!agentData) {
        MessageManager.error(t("Failed to build agent data"));
      ...
      if (keyAssignOption === "create_new" && newKeyName) {
        ...
        if (!selectedExistingKey) {
          MessageManager.error(t("Please select an existing key to assign"));
          setIsSubmitting(false);
          return;
        }
      ...
      MessageManager.error(
        errorMessage ? t("Failed to create agent: {0}", errorMessage) : t("Failed to create agent"),
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const renderEntitlementsStep = () => (
    <div className="space-y-4">
      ...
      <Form.Item
        label={<span className="text-sm font-medium text-gray-700">{t("Allowed Models")}</span>}
        name="entitlement_models"
        tooltip={t("Restrict which models this agent can call. Leave empty to allow all.")}
      >
        <Select
          mode="tags"
          style={{ width: "100%" }}
          placeholder={loadingModels ? t("Loading models...") : t("Select models (leave empty for all)")}
          tokenSeparators={[","]}
      ...
      <Form.Item
        label={<span className="text-sm font-medium text-gray-700">{t("Allowed Agents (Sub-Agents)")}</span>}
        name="entitlement_agents"
        tooltip={t("Restrict which other agents this agent can invoke as sub-agents. Leave empty to allow all.")}
      >
        <Select
          mode="multiple"
          style={{ width: "100%" }}
          placeholder={loadingAgents ? t("Loading agents...") : t("Select agents (leave empty for all)")}
      ...
      <Form.Item
        label={
          <span>
            ...
            <InfoCircleOutlined
              title={t("Select which MCP servers or access groups this agent can access")}
  ...
  const renderObservabilityStep = () => (
    <div className="space-y-6">
      <div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-gray-700">
                {t("Require x-litellm-trace-id on calls TO this agent")}
              </span>
              <p className="text-xs text-gray-500 mt-1">
                {t("Only accept this agent being invoked with a trace-id (e.g. when used as a sub-agent).")}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-gray-700">
                {t("Require x-litellm-trace-id on calls BY this agent")}
              </span>
              <p className="text-xs text-gray-500 mt-1">
                {t("Requires LLM/MCP calls made by this agent to include x-litellm-trace-id for session tracking.")}
              </p>
            </div>
            <Switch
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-3">{t("Budgets & Rate Limits")}</h4>
        <div className="space-y-4">
          ...
          <div className="text-sm font-medium text-gray-700">{t("Session Budgets")}</div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-gray-600 block mb-1">{t("Max Iterations")}</label>
              <InputNumber
                className="w-full"
                min={1}
              ...
              />
              <p className="text-xs text-gray-400 mt-1">{t("Hard cap on LLM calls per session")}</p>
            </div>
            <div>
              <label className="text-sm text-gray-600 block mb-1">{t("Max Budget Per Session ($)")}</label>
              <InputNumber
                className="w-full"
                min={0.01}
                step={0.5}
              ...
              />
              <p className="text-xs text-gray-400 mt-1">{t("Max spend per trace before returning 429")}</p>
            </div>
          </div>

          <Divider className="my-2" />

          <div className="text-sm font-medium text-gray-700">{t("Agent Rate Limits")}</div>
          <p className="text-xs text-gray-500">{t("Global rate limits applied across all callers of this agent.")}</p>
          <div className="grid grid-cols-2 gap-4">
            <Form.Item label={t("TPM Limit")} name="tpm_limit" className="mb-0">
              <InputNumber
                className="w-full"
                min={0}
            ...
            <Form.Item label={t("RPM Limit")} name="rpm_limit" className="mb-0">
              <InputNumber className="w-full" min={0} placeholder={t("e.g. 100")} disabled={!requireTraceIdOutbound} />
            </Form.Item>
          </div>

          <div className="text-sm font-medium text-gray-700 mt-4">{t("Per-Session Rate Limits")}</div>
          <p className="text-xs text-gray-500">
            {t("Rate limits per session (x-litellm-trace-id). Each session gets its own counters.")}
          </p>
          <div className="grid grid-cols-2 gap-4">
            <Form.Item label={t("Session TPM Limit")} name="session_tpm_limit" className="mb-0">
              <InputNumber className="w-full" min={0} placeholder={t("e.g. 10000")} disabled={!requireTraceIdOutbound} />
            </Form.Item>
            <Form.Item label={t("Session RPM Limit")} name="session_rpm_limit" className="mb-0">
              <InputNumber className="w-full" min={0} placeholder={t("e.g. 20")} disabled={!requireTraceIdOutbound} />
            </Form.Item>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mb-3">
          {t("Apply guardrails to this agent. Selected guardrails will run on all calls made by this agent.")}
        </p>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-amber-700">{t("Custom / Other")}</span>
                      <Tag color="orange" style={{ fontSize: 10, padding: "0 4px" }}>
                        GENERIC
                      </Tag>
                    </div>
                    <div className="text-xs text-amber-600">
                      {t("For agents that don't follow a standard protocol — just needs a virtual key")}
                    </div>
                  </div>
                </div>
              </div>
        ...
        {agentType === CUSTOM_AGENT_TYPE ? (
          <div className="space-y-4">
            <Form.Item
              label={t("Agent Name")}
              name="agent_name"
              rules={[{ required: true, message: t("Please enter an agent name") }]}
            >
              <Input placeholder={t("e.g. my-custom-agent")} />
            </Form.Item>
            <Form.Item label={t("Description")} name="description">
              <Input.TextArea placeholder={t("Describe what this agent does…")} rows={3} />
            </Form.Item>
          </div>
        ) : agentType === "a2a" ? (
  ...
  const renderAssignKeyStep = () => {
    const agentName = form.getFieldValue("agent_name") || t("your-agent");
    return (
      <div>
        {/* Agent name chip */}
        ...
        <Form.Item
          label={<span className="text-sm font-medium text-gray-700">{t("Assign to Team")}</span>}
          name="team_id"
          tooltip={t("Optionally assign this agent to a team. The agent and its key will belong to the selected team.")}
        >
          <TeamDropdown />
        </Form.Item>
              ...
              <div className="flex items-start gap-3 flex-1">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-gray-900">{t("Create a new key for this agent")}</span>
                  </div>
                  <p className="text-sm text-gray-500 mt-1">{t("A dedicated key scoped to this agent.")}</p>
                  {keyAssignOption === "create_new" && (
                    <div className="mt-3 space-y-3" onClick={(e) => e.stopPropagation()}>
                      <div>
                        <label className="text-sm text-gray-600 block mb-1">{t("Key Name")}</label>
                        <Input
                          value={newKeyName}
                          onChange={(e) => setNewKeyName(e.target.value)}
                          placeholder={t("e.g. my-agent-key")}
                        />
                      </div>
                    </div>
                  )}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-gray-900">{t("Assign an existing key")}</span>
                </div>
                <p className="text-sm text-gray-500 mt-1">{t("Re-assign a key you already have to this agent.")}</p>
                {keyAssignOption === "existing_key" && (
                  <div className="mt-3" onClick={(e) => e.stopPropagation()}>
                    <Select
                      showSearch
                      style={{ width: "100%" }}
                      placeholder={t("Search by key name…")}
                      loading={loadingKeys}
        ...
        <div className="text-center mt-4">
          ...
          >
            {t("Skip for now — I'll assign a key later")}
          </button>
        </div>
      </div>
    );
  ...
  const renderReadyStep = () => (
    <div className="text-center py-6">
      ...
      <h3 className="text-xl font-semibold text-gray-900 mb-2">{t("Agent Created!")}</h3>
      ...
      {assignedKeyAlias && (
        <p className="text-sm text-gray-600 mt-2">
          {t("Key {0} has been assigned to this agent.", assignedKeyAlias)}
        </p>
      )}
      {!createdKeyValue && !assignedKeyAlias && keyAssignOption === "skip" && (
        <p className="text-sm text-gray-500 mt-2">{t("No key assigned. You can create one from the Virtual Keys page.")}</p>
      )}
    </div>
  );
