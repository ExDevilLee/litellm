...
const GuardrailSelectionModal: React.FC<GuardrailSelectionModalProps> = ({
  ...
  const guardrailsInfo: GuardrailInfo[] = (template?.guardrailDefinitions || []).map((def: any) => ({
    ...
    description: def.guardrail_info?.description || t("No description available"),
      ...
      <DialogContent className="sm:max-w-175">
          ...
          <DialogTitle className="flex items-center gap-2 text-lg">
            ...
            {progressInfo && (
              <Badge variant="secondary">
                {t("Template {0} of {1}", progressInfo.current, progressInfo.total)}
              </Badge>
            )}
          </DialogTitle>
        ...
        <div className="py-4">
          ...
          <div className="mb-4 flex items-center gap-4 rounded-lg border border-border bg-muted p-3">
            ...
            <div className="flex-1">
              <div className="text-sm">
                <span className="font-medium">{t("{0} total guardrails", guardrailsInfo.length)}</span>
                <span className="mx-2 text-muted-foreground">•</span>
                <span className="font-medium text-green-600">{t("{0} new", newGuardrailsCount)}</span>
                {existingCount > 0 && (
                  <>
                    <span className="mx-2 text-muted-foreground">•</span>
                    <span className="text-muted-foreground">{t("{0} already exist", existingCount)}</span>
                  </>
                )}
              </div>
            </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      <Badge variant="secondary">{guardrail.definition?.litellm_params?.mode || t("unknown")}</Badge>
                      {guardrail.definition?.litellm_params?.patterns && (
                        <Badge variant="secondary">
                          {t("{0} pattern(s)", guardrail.definition.litellm_params.patterns.length)}
                        </Badge>
                      )}
                      {guardrail.definition?.litellm_params?.categories && (
                        <Badge variant="secondary">
                          {t("{0} category/categories", guardrail.definition.litellm_params.categories.length)}
                        </Badge>
                      )}
                    </div>
                  </div>
              ...
              <div className="rounded-lg border border-border bg-muted p-3">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">
                    {t("AI-Discovered Competitors ({0})", template.discoveredCompetitors.length)}
                  </span>
                </div>
          ...
          <div className="text-sm text-muted-foreground">
            {selectedCount > 0 ? (
              <p>
                <span className="font-medium text-foreground">{selectedCount}</span> {t("guardrail(s) will be created")}
              </p>
            ) : existingCount > 0 ? (
              <p className="text-green-600">{t("All guardrails already exist. You can proceed to use this template.")}</p>
            ) : (
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={isLoading || (selectedCount === 0 && existingCount === 0)}>
            {selectedCount > 0
              ? t("Create {0} Guardrail(s) & Use Template", selectedCount)
              : t("Use Template")}
          </Button>
        </DialogFooter>
      </DialogContent>
