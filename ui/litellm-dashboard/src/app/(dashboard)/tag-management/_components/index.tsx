...
const TagManagement: React.FC<TagProps> = ({ accessToken, userID, userRole }) => {
  ...
  const fetchTags = async () => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("Error fetching tags: {0}", String(error)));
    } finally {
      setIsLoadingTags(false);
    }
  };
  ...
  const handleCreate = async (formValues: any) => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("Error creating tag: {0}", String(error)));
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("Error deleting tag: {0}", String(error)));
    } finally {
      setIsDeleting(false);
    ...
    if (userID && userRole && accessToken) {
      const fetchModels = async () => {
        try {
          ...
          NotificationsManager.fromBackend(t("Error fetching models: {0}", String(error)));
        }
      };
      fetchModels();
    }
      ...
      {selectedTagId ? (
        ...
        <div className="mt-2 h-[75vh] w-full gap-2 p-8">
          <div className="mt-2 mb-4 flex w-full items-center justify-between">
            <h1>{t("Tag Management")}</h1>
            <div className="flex items-center space-x-2">
              {lastRefreshed && <p className="text-sm">{t("Last Refreshed: {0}", lastRefreshed)}</p>}
              <Button variant="outline" size="icon-sm" aria-label={t("Refresh tags")} onClick={handleRefreshClick}>
                <RefreshCw />
              </Button>
            </div>
          </div>
          ...
          <div className="mb-4 text-sm">
            {t("Click on a tag name to view and edit its details.")}
            <p>
              {t("You can use tags to restrict the usage of certain LLMs based on tags passed in the request. Read more about tag routing")}{" "}
          ...
          <DeleteResourceModal
            ...
            resourceInformationTitle={t("Tag Information")}
            resourceInformation={[{ label: t("Tag Name"), value: tagToDelete, code: true }]}
            onCancel={() => {
              setIsDeleteModalOpen(false);
              setTagToDelete(null);
            }}
